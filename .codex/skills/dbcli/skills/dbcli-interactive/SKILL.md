...
# MySQL
dbcli -c "Server=localhost;Database=mydb;Uid=root;Pwd=pass" -t mysql interactive

# PostgreSQL
dbcli -c "Host=localhost;Database=mydb;Username=postgres;Password=pass" -t postgresql interactive

# DaMeng (达梦)
dbcli -c "Server=localhost;User Id=SYSDBA;PWD=SYSDBA;DATABASE=mydb" -t dm interactive

# GaussDB (华为)
dbcli -c "Host=localhost;Port=8000;Database=mydb;Username=gaussdb;Password=pass" -t gaussdb interactive
```
...
- **dbcli-query**: One-off SELECT queries
- **dbcli-exec**: One-off INSERT/UPDATE/DELETE
- **dbcli-db-ddl**: One-off DDL operations
- **dbcli-tables**: List tables (also available as `.tables` in interactive)
- **dbcli-export**: Export data (also available as `.export` in interactive)
