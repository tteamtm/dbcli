...
# MySQL
dbcli -c "Server=localhost;Database=mydb;Uid=root;Pwd=pass" -t mysql -f table tables

# PostgreSQL
dbcli -c "Host=localhost;Database=mydb;Username=postgres;Password=pass" -t postgresql -f table tables

# Oracle
dbcli -c "Data Source=localhost:1521/orcl;User Id=system;Password=oracle" -t oracle -f table tables
...
```bash
# DaMeng (达梦)
dbcli -c "Server=localhost;User Id=SYSDBA;PWD=SYSDBA;DATABASE=mydb" -t dm -f table tables

# KingbaseES (人大金仓)
dbcli -c "Server=localhost;Port=54321;UID=system;PWD=system;database=mydb" -t kdbndp -f table tables

# GaussDB (华为)
dbcli -c "Host=localhost;Port=8000;Database=mydb;Username=gaussdb;Password=pass" -t gaussdb -f table tables
```
...
- **dbcli-query**: Execute SELECT queries on tables
- **dbcli-exec**: Execute INSERT/UPDATE/DELETE on tables
- **dbcli-db-ddl**: Create, alter, or drop tables
- **dbcli-export**: Export table data
- **dbcli-interactive**: Interactive mode for exploring databases
