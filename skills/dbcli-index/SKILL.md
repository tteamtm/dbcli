...
#### DaMeng (达梦)
```bash
dbcli -c "Server=localhost;User Id=SYSDBA;PWD=SYSDBA;DATABASE=mydb" -t dm ddl "
CREATE INDEX idx_orders_customer ON orders(customer_id)"

# Unique index
dbcli -c "Server=localhost;User Id=SYSDBA;PWD=SYSDBA" -t dm ddl "
CREATE UNIQUE INDEX idx_products_code ON products(product_code)"
```

#### GaussDB (华为)
```bash
dbcli -c "Host=localhost;Port=8000;Database=mydb;Username=gaussdb;Password=pass" -t gaussdb ddl "
...
#### KingbaseES (人大金仓)
```bash
dbcli -c "Server=localhost;Port=54321;Database=mydb;UID=system;PWD=pass" -t kdbndp ddl "
