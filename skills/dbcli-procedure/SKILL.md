...
dbcli -c "Server=.;Database=mydb" -t sqlserver procedure "GetUserById" -p '{"UserId":1}'
...
dbcli [options] procedure "<PROC_NAME>" [-p JSON] [-P params.json]

# Execute procedure and return result set
dbcli [options] procedure-query "<PROC_NAME>" [-p JSON] [-P params.json]
```
