...
# MySQL
dbcli -c "Server=localhost;Database=mydb;Uid=root;Pwd=pass" -t mysql export Customers > Customers_backup_${TIMESTAMP}.sql

# PostgreSQL
dbcli -c "Host=localhost;Database=mydb;Username=postgres;Password=pass" -t postgresql export Orders > Orders_backup_${TIMESTAMP}.sql
```
...
```bash
# Export from MySQL
dbcli -c "Server=source;Database=mydb;Uid=root;Pwd=pass" -t mysql export Users > Users_export.sql

# Import to PostgreSQL (may need SQL syntax adjustments)
dbcli -c "Host=target;Database=mydb;Username=postgres;Password=pass" -t postgresql exec -F Users_export.sql
```
...
# Export table
dbcli -c "Server=localhost;User Id=SYSDBA;PWD=SYSDBA;DATABASE=mydb" -t dm export dm_test > dm_test_backup_${TIMESTAMP}.sql
...
if [ "$confirm" = "yes" ]; then
    dbcli -c "Server=localhost;User Id=SYSDBA;PWD=SYSDBA;DATABASE=mydb" -t dm exec "UPDATE dm_test SET status = 1"
fi
```
...
```bash
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
CONNECTION="Host=localhost;Port=8000;Database=mydb;Username=gaussdb;Password=pass"
...
- **dbcli-query**: Query tables to preview data before export
- **dbcli-exec**: Execute modifications with exported backups
- **dbcli-db-ddl**: DDL operations requiring export backups
- **dbcli-tables**: List tables to export
- **dbcli-interactive**: Interactive mode for selective exports
