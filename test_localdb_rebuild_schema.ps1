...
try {
  ...
  & $DbCli ddl "CREATE DATABASE [$dbName]" -t sqlserver -c $csMaster -f json | Out-Null
...
'@
  & $DbCli ddl $tablesSql -t sqlserver -c $csDb -f json | Out-Null
  if ($LASTEXITCODE -ne 0) { throw "Create tables failed" }

  Write-Step "Create schema objects (index/view/proc/func/trigger)"
  & $DbCli ddl "CREATE INDEX IX_Products_Name ON Products(Name);" -t sqlserver -c $csDb -f json | Out-Null
  if ($LASTEXITCODE -ne 0) { throw "Create index failed" }

  & $DbCli ddl "CREATE VIEW vProducts AS SELECT Id, Name, Price, Stock, CreatedAt FROM Products;" -t sqlserver -c $csDb -f json | Out-Null
  if ($LASTEXITCODE -ne 0) { throw "Create view failed" }

  & $DbCli ddl "CREATE OR ALTER PROCEDURE sp_GetProducts AS BEGIN SET NOCOUNT ON; SELECT * FROM Products ORDER BY Id; END" -t sqlserver -c $csDb -f json | Out-Null
  if ($LASTEXITCODE -ne 0) { throw "Create procedure failed" }

  & $DbCli ddl "CREATE OR ALTER FUNCTION fn_AddOne(@x INT) RETURNS INT AS BEGIN RETURN @x + 1; END" -t sqlserver -c $csDb -f json | Out-Null
  if ($LASTEXITCODE -ne 0) { throw "Create function failed" }

  & $DbCli ddl "CREATE OR ALTER TRIGGER tr_Products_Insert ON Products AFTER INSERT AS BEGIN SET NOCOUNT ON; END" -t sqlserver -c $csDb -f json | Out-Null
  if ($LASTEXITCODE -ne 0) { throw "Create trigger failed" }

  Write-Step "Insert data"
  & $DbCli exec "INSERT INTO Products (Name, Price, Stock) VALUES ('Apple', 1.99, 100);" -t sqlserver -c $csDb -f json | Out-Null
  & $DbCli exec "INSERT INTO Products (Name, Price, Stock) VALUES ('Banana', 0.99, 150);" -t sqlserver -c $csDb -f json | Out-Null
  & $DbCli exec "INSERT INTO Orders (ProductId, Qty) VALUES (1, 2);" -t sqlserver -c $csDb -f json | Out-Null
  & $DbCli exec "INSERT INTO Orders (ProductId, Qty) VALUES (2, 5);" -t sqlserver -c $csDb -f json | Out-Null
  if ($LASTEXITCODE -ne 0) { throw "Insert data failed" }

  Write-Step "Export schema objects to file"
  & $DbCli export-schema all -t sqlserver -c $csDb -o $schemaObjectsFile | Out-Null
  ...
  $out = & $DbCli export-schema all -t sqlserver -c $csDb --output-dir $schemaDir
  ...
  $out = & $DbCli backup Products -o $backupProducts -t sqlserver -c $csDb -f json
  if ($LASTEXITCODE -ne 0) { throw "backup Products failed. Output: $out" }

  $out = & $DbCli backup Orders -o $backupOrders -t sqlserver -c $csDb -f json
  if ($LASTEXITCODE -ne 0) { throw "backup Orders failed. Output: $out" }

  Write-Step "Drop schema objects + tables (keep backups)"
  # Drop dependents first
  & $DbCli ddl "DROP TRIGGER IF EXISTS tr_Products_Insert;" -t sqlserver -c $csDb -f json | Out-Null
  & $DbCli ddl "DROP PROCEDURE IF EXISTS sp_GetProducts;" -t sqlserver -c $csDb -f json | Out-Null
  & $DbCli ddl "DROP FUNCTION IF EXISTS fn_AddOne;" -t sqlserver -c $csDb -f json | Out-Null
  & $DbCli ddl "DROP VIEW IF EXISTS vProducts;" -t sqlserver -c $csDb -f json | Out-Null
  & $DbCli ddl "DROP INDEX IF EXISTS IX_Products_Name ON Products;" -t sqlserver -c $csDb -f json | Out-Null

  # Drop tables (FK order)
  & $DbCli ddl "DROP TABLE IF EXISTS Orders; DROP TABLE IF EXISTS Products;" -t sqlserver -c $csDb -f json | Out-Null
  if ($LASTEXITCODE -ne 0) { throw "Drop objects/tables failed" }

  Write-Step "Rebuild from script file"
  $out = & $DbCli ddl --file $rebuildFile -t sqlserver -c $csDb -f json
  if ($LASTEXITCODE -ne 0) { throw "Rebuild ddl failed (script execution). Output: $out" }

  Write-Step "Restore data"
  $out = & $DbCli restore Products --from $backupProducts -t sqlserver -c $csDb -f json
  if ($LASTEXITCODE -ne 0) { throw "restore Products failed. Output: $out" }

  $out = & $DbCli restore Orders --from $backupOrders -t sqlserver -c $csDb -f json
  if ($LASTEXITCODE -ne 0) { throw "restore Orders failed. Output: $out" }

  Write-Step "Verify counts and objects"
  $prodCnt = (& $DbCli query "SELECT COUNT(*) AS Cnt FROM Products" -t sqlserver -c $csDb -f json | ConvertFrom-Json)[0].Cnt
  $orderCnt = (& $DbCli query "SELECT COUNT(*) AS Cnt FROM Orders" -t sqlserver -c $csDb -f json | ConvertFrom-Json)[0].Cnt
