...
.EXAMPLE
  $env:DBCLI = '.\dist-win-x64\dbcli.exe'; .\test_localdb.ps1
#>
...
'@
  ...
  $r = & $DbCli exec "DELETE FROM Products WHERE Name='Banana'" -t sqlserver -c $csDb -f json
  if ($LASTEXITCODE -ne 0) { throw "delete failed. Output: $r" }
