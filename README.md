...
Works seamlessly with SQLite, PostgreSQL, MySQL, Oracle, Microsoft SQL Server,
Chinese domestic databases (DaMeng, KingbaseES, Oscar, HighGo, GaussDB, GBase), and more
```
...
# List views
dbcli -c "Server=localhost;Database=mydb;User Id=SYSDBA;Password=xxxxxxxxxx" -t dm views --scope all --owner DOC -f table
...
# MySQL
dbcli -c "Server=localhost;Database=mydb;Uid=root;Pwd=xxxxxxxxxx" query "SELECT * FROM Users LIMIT 10" -t mysql

# PostgreSQL
dbcli -c "Host=localhost;Database=mydb;Username=postgres;Password=xxxxxxxxxx" query "SELECT * FROM Users LIMIT 10" -t postgresql
```
